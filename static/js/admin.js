...
function getCurrentQuestions(formType) {
    ...
    try {
        ...
        questionItems.forEach((item, index) => {
            ...
            const dividerElement = item.querySelector('.divider-question');
            if (dividerElement) {
                question.type = 'divider';
                const titleInput = item.querySelector('input[type="text"]');
                question.title = titleInput ? titleInput.value : '';
            } else {
                // Regular question - extract all form data
                const regularElement = item.querySelector('.regular-question');
                if (regularElement) {
                    // Extract type
                    ...
                    const requiredSelect = item.querySelectorAll('select')[1];
                    if (requiredSelect) {
                    ...
                    // Extract destination column(s) based on question type
                    const inputs = item.querySelectorAll('input[type="text"]');

                    if (question.type === 'dropdown') {
                        // Extract dropdown options
                        const optionsInput = Array.from(inputs).find(input => 
                            input.placeholder && input.placeholder.includes('Opção'));
                        if (optionsInput) {
                    ...
                    if (question.type === 'monday_column') {
                        // Extract source column, text destination, and rating destination
                        inputs.forEach(input => {
                            const placeholder = input.placeholder || '';
                            const label = input.closest('.col-md-6, .col-12')?.querySelector('label')?.textContent || '';

                            if (placeholder.includes('text_mkrj') || placeholder.includes('dropdown_mk') || label.includes('Origem')) {
                                question.source_column = input.value;
                            } else if (label.includes('Texto') || label.includes('nome')) {
                                question.text_destination_column = input.value;
                            } else if (label.includes('Avaliação') || label.includes('nota')) {
                                question.rating_destination_column = input.value;
                            }
                        });
                    } else {
                        // For other question types, find destination column
                        const destInput = Array.from(inputs).find(input => 
                            input.placeholder && (input.placeholder.includes('text_mk') || input.placeholder.includes('numeric_mk') || input.placeholder.includes('long_text_mk'))
                        );
                        if (destInput) {
                            question.destination_column = destInput.value;
                        }
                    }

                    // Extract conditional logic
                    const conditionalSelects = item.querySelectorAll('.conditional-depends-on, .conditional-show-if');
                    if (conditionalSelects.length >= 2) {
                        const dependsOnSelect = item.querySelector('.conditional-depends-on');
                        const showIfSelect = item.querySelector('.conditional-show-if');

                        if (dependsOnSelect && dependsOnSelect.value) {
                            question.conditional = {
                                depends_on: dependsOnSelect.value,
                                show_if: showIfSelect ? showIfSelect.value : ''
                            };
        ...
        console.log(`Extracted ${questions.length} questions for ${formType}:`, questions);
        return questions;
