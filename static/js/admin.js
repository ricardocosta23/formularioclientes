...
function showDividerInput(button, formType, index) {
    const dividerContainer = button.closest('.divider-container');

    // Create input field for divider title
    const inputContainer = document.createElement('div');
    inputContainer.className = 'divider-input-container';
    ...
    inputContainer.innerHTML = `
        <div style="margin-bottom: 10px;">
            <label style="display: block; margin-bottom: 5px; font-weight: 500;">Título do Divisor:</label>
            <input type="text" class="form-control" id="divider-title-input" placeholder="Ex: ✏️ Dados Pessoais" style="width: 100%;">
        </div>
        <div style="display: flex; gap: 10px; justify-content: flex-end;">
            <button type="button" class="btn btn-secondary btn-sm" onclick="cancelDividerInput(this)">Cancelar</button>
            <button type="button" class="btn btn-primary btn-sm" onclick="addDivider('${formType}', ${index}, this)">Adicionar</button>
        </div>
    `;

    // Position the container relative to the button
    const buttonRect = button.getBoundingClientRect();
    const parentRect = dividerContainer.getBoundingClientRect();

    dividerContainer.style.position = 'relative';
    dividerContainer.appendChild(inputContainer);

    // Focus on the input
    const titleInput = inputContainer.querySelector('#divider-title-input');
    titleInput.focus();

    // Handle Enter key
    titleInput.addEventListener('keypress', function(e) {
        if (e.key === 'Enter') {
            addDivider(formType, index, inputContainer.querySelector('.btn-primary'));
        }
    });
    ...
    titleInput.addEventListener('keydown', function(e) {
        if (e.key === 'Escape') {
            cancelDividerInput(inputContainer.querySelector('.btn-secondary'));
        }
    });
}

function cancelDividerInput(button) {
    const inputContainer = button.closest('.divider-input-container');
    if (inputContainer) {
        inputContainer.remove();
    }
}

function addDivider(formType, index, button) {
    const inputContainer = button.closest('.divider-input-container');
    const titleInput = inputContainer.querySelector('#divider-title-input');
    const title = titleInput.value.trim();

    if (!title) {
        alert('Por favor, insira um título para o divisor.');
        titleInput.focus();
        return;
    }
    ...
    // Add divider at the specified index
    if (index === -1) {
        // Add at the end
        currentQuestions.push(newDivider);
    } else {
        // Insert at specific position
        currentQuestions.splice(index + 1, 0, newDivider);
    }
    ...
    console.log('Added divider:', newDivider);
}
